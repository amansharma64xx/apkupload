...
# Yaha pe dusre telegram wale ka repo daal diya hai
GITHUB_REPO = os.environ.get("GITHUB_REPO", "amansharma64xx/MParivahan")
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"• I will save it as `{FIXED_FILE_NAME}` on the cloud\n"
        f"• The old file will be replaced, the link always stays the same\n\n"
        f"📂 *Storage:* Cloud Server\n"
        f"🌐 *Status:* Active\n\n"
...
async def cmd_status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    msg = await update.message.reply_text("🔍 Checking server connection...")
    ...
    if response.status_code == 200:
        data = response.json()
        # GitHub returns size in bytes, convert to MB
        ...
        await msg.edit_text(
            ...
            f"Please contact the admin to check the configuration.",
            parse_mode="Markdown"
        )
...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await status_msg.edit_text(
            f"❌ *Upload Failed!*\n\n"
            f"Could not upload to the cloud.\n"
            f"Error: `{str(e)}`",
            parse_mode="Markdown"
        )
        return
    ...
    if success:
        ...
        await status_msg.edit_text(
            f"❌ *Upload Failed!*\n\n"
            f"Possible reasons:\n"
            f"• Server access key expired or invalid\n"
            f"• Storage location not reachable\n"
...
def main():
    ...
    logger.info(f"Bot started — pushing APKs to {GITHUB_REPO}@{GITHUB_BRANCH}/{FIXED_FILE_NAME}")
